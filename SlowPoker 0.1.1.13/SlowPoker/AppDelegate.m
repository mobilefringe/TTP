...
@synthesize inviteFacebookFriendsViewController;
...
-(void)pushToFacebookInvite{
    if(!inviteFacebookFriendsViewController){
        self.inviteFacebookFriendsViewController = [[InviteFacebookFriendsViewController alloc] initWithNibName:nil bundle:nil];
    }
    [self.navController pushViewController:inviteFacebookFriendsViewController animated:YES];
}
...
-(void)showImage:(NSString *)userId{
    if(!showImage){
        self.showImage = [[Avatar alloc] initWithFrame:self.window.bounds];
        showImage.imageView.contentMode = UIViewContentModeScaleAspectFit;
        showImage.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.9];
        UIButton *closeButton = [UIButton buttonWithType:UIButtonTypeCustom];
        closeButton.frame = showImage.bounds;
        closeButton.showsTouchWhenHighlighted = YES;
        [closeButton addTarget:self action:@selector(hideImage) forControlEvents:UIControlEventTouchUpInside];
        [showImage addSubview:closeButton];
        [self.window addSubview:showImage];
        showImage.alpha = 0;
    }
    [showImage loadAvatar:userId];
...
-(void)showAchievement:(NSMutableDictionary *)achievement{
    ...
    if ([[UIScreen mainScreen] bounds].size.height > 480.0f){
        achievementEarnedView.center = CGPointMake(160, 640);
    }
    ...
    if ([[UIScreen mainScreen] bounds].size.height > 480.0f){
        achievementEarnedView.center = CGPointMake(160, 530);
    }else{
...
-(void)emailBug:(UIViewController *)viewController{
    ...
    if(picker){
        ...
        UIAlertView *noEmail = [[UIAlertView alloc] initWithTitle:@"No Email Account" message:@"There is no email account setup on this device. Please set one up to email feedback." delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [noEmail show];
    }
}
...
-(void)finishAutoLogin:(NSMutableDictionary *)loginResults{
    ...
    if([@"1" isEqualToString:[loginResults objectForKey:@"success"]]){
    ...
    }else if([@"0" isEqualToString:[loginResults objectForKey:@"success"]]){
        logginAttempts = 0;
        if(facebook){
            [facebook logout];
        }
        [self showInfoViewController:navController];
        ...
        if(logginAttempts < 3){
            ...
            logginAttempts = 0;
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Unable to Login" message:@"Please check your internet connection and try again. Thank you." delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            [alert show];
        }
    }
}
...
-(void)showInfoViewController:(UIViewController *)parentViewController{
    
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    if([[prefs valueForKey:@"SHOWN_INFO"] isEqualToString:@"YES"]){
        return;
    }
    ...
    [parentViewController presentModalViewController:infoViewController animated:YES];
...
-(void)closeInfo{
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    [prefs setValue:@"YES" forKey:@"SHOWN_INFO"];
    [prefs synchronize];
