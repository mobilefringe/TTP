...
@synthesize loadFromLocalFile;
@synthesize isGamesUpdating;
@synthesize showHand;
...
-(void)processLogin:(NSMutableDictionary *)results playerData:(NSMutableDictionary *)playerData{
    ...
    if([@"1" isEqualToString:[results objectForKey:@"success"]]){
        ...
        //sort your friends.
        NSSortDescriptor *sortDescriptor = [NSSortDescriptor sortDescriptorWithKey: @"userName" ascending: YES selector: @selector(caseInsensitiveCompare:)];
        ...
        NSArray *sortedArray = [myFriends sortedArrayUsingDescriptors:sortDescriptors];
        self.myFriends = [NSMutableArray arrayWithArray:sortedArray];
        ...
        NSString *facebookID = [results valueForKey:@"facebook_id"];
        ...
        NSMutableDictionary *earnedAchievement2 = [self getGameAchievementForCode:@"REGISTER_FACEBOOK" category:@"BLACK"];
        if(facebookID && [facebookID length] > 0 && ![self currentUserHasAchievement:earnedAchievement2] && ![facebookID isEqualToString:[prefs valueForKey:@"FACEBOOK_REGISTERED"]]){
            [prefs setValue:facebookID forKey:@"FACEBOOK_REGISTERED"];
            //[self logAchievements];
            [self incrementAchievement:earnedAchievement2 forUser:myUserID];
        }
...
-(void)reloadCurrentGame{
    ...
    NSMutableDictionary *userDict = [[NSMutableDictionary alloc] init];
    [userDict setValue:currentGameID forKey:@"gameID"];
    NSError *error;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:userDict options:NSJSONWritingPrettyPrinted error:&error];
...
-(void)arrangeUserGames{
    ...
    for (NSMutableDictionary *game in games) {
        ...
        if([self hasMeInGame:game]){
            if([self cardsDidChange:game] || [self isGameActive:game]){
                if([self cardsDidChange:game] || [self isMyTurn:game] || [self isPlayerMePendingForGame:game]){
                    [yourTurnGames addObject:game];
                }else{
                    [theirTurnGames addObject:game];
                }
            }else if([self isGamePending:game]){
                if([self isPlayerMePendingForGame:game] || [self hasEnoughPlayersToStart:game]){//([self hasEnoughPlayersToStart:game] && [[game valueForKey:@"gameOwnerID"] isEqualToString:myUserID])){
                    [yourTurnGames addObject:game];
                }else{
                    [theirTurnGames addObject:game];
                }
...
-(BOOL)cardsDidChange:(NSMutableDictionary *)game{
    ...
    //NSLog(@"prefs cards:%@",[prefs valueForKey:[game valueForKey:@"gameID"]]);
    if([[prefs valueForKey:[game valueForKey:@"gameID"]] isEqualToString:@"(null)_(null)"]){
        return NO;
    }
    if(![[prefs valueForKey:[game valueForKey:@"gameID"]] isEqualToString:playerCards]){
        ...
        //NSLog(@"oldCardOne:%@",[cards objectAtIndex:0]);
        //NSLog(@"oldCardTwo:%@",[cards objectAtIndex:1]);
       ...
       // NSLog(@"game:%@",game);
        if((![[myPlayer valueForKey:@"status"] isEqualToString:@"playing"] || ![[game valueForKey:@"status"] isEqualToString:@"active"]) && ![self hasLatestHandSummaryBeenShownForGame:game]){
            [game setValue:@"YES" forKey:@"cardsDidChange"];
            NSArray *cards = [[prefs valueForKey:[game valueForKey:@"gameID"]] componentsSeparatedByString:@"_"];
            [game setValue:[cards objectAtIndex:0] forKey:@"oldCardOne"];
            [game setValue:[cards objectAtIndex:1] forKey:@"oldCardTwo"];
...
-(BOOL)hasLatestHandSummaryBeenShownForGame:(NSMutableDictionary *)game{
    ...
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSString *summaryKey = [NSString stringWithFormat:@"SUMMARY_%@_%@",[game valueForKey:@"gameID"],[DataManager sharedInstance].myUserID];
    
    
    int handNumberSeen = [[prefs valueForKey:summaryKey] intValue];
    ...
    int myLastBetHandNumber = [[myPlayerState valueForKey:@"lastBetHandNumber"] intValue];

    if(myLastBetHandNumber > handNumberSeen){
        return NO;
    }
...
-(void)postSystemMessageForCurrentGame:(NSString *)message{
    [self postSystemMessageForCurrentGameWithSystemID:message systemID:@"-1"];
}
...
-(void)postSystemMessageForCurrentGameWithSystemID:(NSString *)message systemID:(NSString *)systemID{
    ...
    [localMessageDict setValue:systemID forKey:@"userID"];
    ...
    dispatch_async(kBgQueue, ^{
        ...
        [messageDict setValue:systemID forKey:@"userID"];
...
-(void)postNudgeToCurrentPlayer{
    ...
    dispatch_async(kBgQueue, ^{
        ...
        [self postSystemMessageForCurrentGame:[NSString stringWithFormat:@"%@ is nudging %@.",myUserName,[self getCurrentPlayersTurn]]];
        ...
        if(currentGame && [currentGame objectForKey:@"gameState"] && [@"active" isEqualToString:[currentGame valueForKey:@"status"]]){
            if([[currentGame objectForKey:@"gameState"] objectForKey:@"hands"]){
                ...
                if([hands count] > 0){
                    ...
                    NSMutableDictionary *currentPlayer = [[DataManager sharedInstance] getCurrentPlayerForCurrentGame];
                    NSMutableDictionary *currentPlayerState = [currentPlayer objectForKey:@"playerState"];
                    [betRound setValue:[currentPlayer valueForKey:@"userName"] forKey:@"userName"];
                    ...
                    [betRound setValue:[currentPlayerState valueForKey:@"lastBetHandState"] forKey:@"lastBetHandState"];
...
-(void)didFinishUpdate:(NSMutableDictionary *)updateResults{
    ...
    if(updateResults && [[updateResults valueForKey:@"success"] isEqualToString:@"1"]){
    //self.isUpdatingGame = NO;
        NSMutableArray *playersForAPN = [[NSMutableArray alloc] init];
        NSString *nextUserID = [currentGame valueForKey:@"nextActionForUserID"];
        if(![nextUserID isEqualToString:myUserID]){
            [playersForAPN addObject:nextUserID];
            NSMutableDictionary *payLoad = [[NSMutableDictionary alloc] init];
            [payLoad setValue:currentGameID forKey:@"gameID"];
        ...
        UIAlertView *cannotConnect = [[UIAlertView alloc] initWithTitle:@"Cannot Connect" message:@"Please make sure you have a connection to the internet and try again." delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [cannotConnect show];
        [self reloadCurrentGame];
    }
}
            ...
            if([[game objectForKey:@"turnState"] objectForKey:@"players"]){
                ...
                for (NSMutableDictionary *player in players) {
                    ...
                    if([[player objectForKey:@"userID"] isEqualToString:[game objectForKey:@"nextActionForUserID"]]){
                        return [NSString stringWithFormat:@"%@",[player objectForKey:@"userName"]];
                    }
                }
...
-(void)postRound:(NSMutableDictionary *)round{
    ...
    //NSLog(@"currentPlayer:%@",[[DataManager sharedInstance] getCurrentPlayerForCurrentGame]);
    NSMutableArray *hands = [[currentGame objectForKey:@"gameState"] objectForKey:@"hands"];
    if([hands count] > 0){
        self.currentHand = [hands objectAtIndex:0];
    }
    ...
    [currentPlayerState setValue:[currentHand valueForKey:@"number"] forKey:@"lastBetHandNumber"];
    ...
    for (NSMutableDictionary *player in players) {
        //NSLog(@"handState:%d, player:%@",handState,player);
    ...
    if(increaseHandState){
        for(NSString *aKey in betAmounts){
            ...
            if(![[betAmounts valueForKey:aKey] isEqualToString:compareAmount] && ![@"fold" isEqualToString:[betAmounts valueForKey:aKey]]){
                NSMutableDictionary *player = [self getPlayerForIDCurrentGame:aKey];
                //NSLog(@"player:%@",player);
                NSMutableDictionary *playerState = [player valueForKey:@"playerState"];
                //NSLog(@"playerState:%@",playerState);
                if([[playerState valueForKey:@"userStack"] doubleValue] > 0 && [[player valueForKey:@"status"] isEqualToString:@"playing"] && ![[playerState valueForKey:@"action"] isEqualToString:@"fold"]){
    ...
    if(increaseHandState){
        ...
        //[betRound setValue:[NSString stringWithFormat:@"%d",handState] forKey:@"lastBetHandState"];
        //[currentPlayerState setValue:[NSString stringWithFormat:@"%d",handState] forKey:@"lastBetHandState"];
    }
    ...
    if(winners && [winners count] > 0){
                    ...
                    if([[winner valueForKey:@"userID"] isEqualToString:[player valueForKey:@"userID"]] && ![[winner valueForKey:@"userID"] isEqualToString:myUserID]){
                        ...
                        if(winAmount > 0){
                            ...
                            if([[winner valueForKey:@"type"] isEqualToString:@"USER_FOLD"]){
                                message = [NSString stringWithFormat:@"%@ won $%.2f! Table folded in game %@.",[winner valueForKey:@"userName"],winAmount,[currentGame valueForKey:@"gameID"]] ;
                            }else{
                                message = [NSString stringWithFormat:@"$%.2f was won in game %@. See who won!",winAmount,[currentGame valueForKey:@"gameID"]] ;
                            }
                            NSMutableDictionary *payLoad = [[NSMutableDictionary alloc] init];
                            [payLoad setValue:currentGameID forKey:@"gameID"];
        ...
        if([[round valueForKey:@"action"] isEqualToString:@"raise"]){
            actionMessage = [NSString stringWithFormat:@"%@ %@ $%@",myUserName,@"raised",[round valueForKey:@"raiseAmount"]]; 
        }else if([[round valueForKey:@"action"] isEqualToString:@"bet"]){
            actionMessage = [NSString stringWithFormat:@"%@ %@ $%@",myUserName,@"bet",[round valueForKey:@"raiseAmount"]]; 
        }else if([[round valueForKey:@"action"] isEqualToString:@"reraise"]){
            actionMessage = [NSString stringWithFormat:@"%@ %@ $%@",myUserName,@"reraised",[round valueForKey:@"raiseAmount"]]; 
        }else if([[round valueForKey:@"action"] isEqualToString:@"call"]){
        ...
        //NSLog(@"actionMessage:%@",actionMessage);
...
-(NSMutableDictionary *)getNextTurnPlayerFromCurrentGame:(BOOL)isFirstActor{
    ...
    //we have gone through all the players and no one else can go, therefor it is the current players turn again. Very rare case but valid.
    for (int i = 0; i < players.count;i++) {
        
        nextPlayer = [players objectAtIndex:i];
        //NSLog(@"nextPlayer:%@",nextPlayer);
        if(currentPlayerOrder == [[nextPlayer objectForKey:@"order"] intValue] && [self canPlayerBet:nextPlayer]){
...
-(void)offerRematch:(NSMutableDictionary *)game{
    ...
    [gameSettings setValue:[game valueForKey:@"gameID"] forKey:@"orginalGameID"];
...
-(void)buyInToCurrentGame:(NSMutableDictionary *)playerState{
    ...
    [self postSystemMessageForCurrentGame:[NSString stringWithFormat:@"%@ has bought in for $%@",myUserName,[playerState valueForKey:@"userStack"]]];
    ...
    if([self isCurrentGameTypeTournament] && [self isCurrentGamePending]){
        ...
        if(numOfPendingPlayers == 0){
            ...
            NSMutableDictionary *payLoad = [[NSMutableDictionary alloc] init];
            [payLoad setValue:currentGameID forKey:@"gameID"];
            [self sendAPN:recipients message:[NSString stringWithFormat:@"Players are ready for game %@. Waiting for you to start tournamant the tournament.",currentGameID] payLoad:payLoad];
        }
        ...
        if(numOfBuyInPlayers >= 2){
            NSMutableArray *recipients = [[NSMutableArray alloc] init];
            [recipients addObject:[currentGame valueForKey:@"gameOwnerID"]];
            NSMutableDictionary *payLoad = [[NSMutableDictionary alloc] init];
            [payLoad setValue:currentGameID forKey:@"gameID"];
            [self sendAPN:recipients message:[NSString stringWithFormat:@"Waiting for you to start cash game %@. At least 2 players have bought in.",currentGameID] payLoad:payLoad];
        }
...
-(void)dealNewHand{
    ...
    //NSLog(@"Dealer Player:%@",dealerPlayer);
    int numOfPlayingPlayers = 0;
    for (NSMutableDictionary *player in players) {
        if([[player objectForKey:@"status"] isEqualToString:@"playing"]){
            numOfPlayingPlayers++;
    ...
    //NSLog(@"smallBlindPlayer:%@",smallBlindPlayer);
    ...
    if(numOfPlayingPlayers > 2){
        ...
        [[smallBlindPlayer valueForKey:@"playerState"] setValue:@"YES" forKey:@"isBigBlind"];
        
        [smallBlindRound setValue:@"0" forKey:@"potSize"];
        [smallBlindRound setValue:@"bigBlind" forKey:@"action"];
        
        
        
        double userStackBigBlind = [[[smallBlindPlayer objectForKey:@"playerState"] objectForKey:@"userStack"] doubleValue];
        ...
        [smallBlindRound setValue:[NSString stringWithFormat:@"%.2f",userStackBigBlind] forKey:@"userStack"];
        [[smallBlindPlayer valueForKey:@"playerState"] setValue:[NSString stringWithFormat:@"%.2f",userStackBigBlind] forKey:@"userStack"];
    ...
    //NSLog(@"bigBlindPlayer:%@",bigBlindPlayer);
    ...
    if(numOfPlayingPlayers > 2){
        ...
        [[bigBlindPlayer valueForKey:@"playerState"] setValue:@"YES" forKey:@"isSmallBlind"];
        
        [bigBlindPlayer setValue:@"0" forKey:@"potSize"];
        [bigBlindPlayer setValue:@"smallBlind" forKey:@"action"];
        double userStackSmallBlind = [[[bigBlindPlayer objectForKey:@"playerState"] objectForKey:@"userStack"] doubleValue];
        if(userStackSmallBlind < smallBlind){
            smallBlind = userStackSmallBlind;
        }
        userStackSmallBlind = userStackSmallBlind - smallBlind;
        
        [bigBlindRound setValue:[NSString stringWithFormat:@"%.2f",userStackSmallBlind] forKey:@"userStack"];
        [[bigBlindPlayer valueForKey:@"playerState"] setValue:[NSString stringWithFormat:@"%.2f",userStackSmallBlind] forKey:@"userStack"];
    ...
    //NSLog(@"nextActionForUserID:%@",[currentGame valueForKey:@"nextActionForUserID"]);
    [self updateCurrentGame];
...
-(NSMutableDictionary *)getNextDealerPlayer{
    ...
    for (NSMutableDictionary *player in players) {
        ...
        if([playerState objectForKey:@"isDealer"] && [@"YES" isEqualToString:[playerState objectForKey:@"isDealer"]] && [[player valueForKey:@"status"] isEqualToString:@"playing"]){
            //NSLog(@"playerState:%@",playerState);
...
-(double)getYourPotCommitment{
    //NSLog(@"currentHand:%@",self.currentHand);
    double potCommitment = 0.00;
    for (NSMutableDictionary *round in [currentHand valueForKey:@"rounds"]) {
        if([[round valueForKey:@"userID"] isEqualToString:myUserID]){
            potCommitment = potCommitment + [[round valueForKey:@"amount"] doubleValue];
        }
    }
    return potCommitment;
}
...
-(BOOL)needsToBuyGame{
    ...
    if([self getNumberOfUnPaidActiveGames] > 5){
        return YES;
    }
    return NO;
}
...
-(int)getLastSeenHandStateForCurrentGame:(NSMutableDictionary *)hand{
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    return [[prefs valueForKey:[NSString stringWithFormat:@"LAST_SEEN_HAND_STATE_%@_%@_%@",myUserID,currentGameID,[hand valueForKey:@"number"]]] intValue];
}

-(void)setLastSeenHandStateForCurrentGame{
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    [prefs setValue:[currentHand valueForKey:@"state"] forKey:[NSString stringWithFormat:@"LAST_SEEN_HAND_STATE_%@_%@_%@",myUserID,currentGameID,[currentHand valueForKey:@"number"]]];
    [prefs synchronize];
...
-(void)postShowHand{
    ...
    [showDict setValue:myUserName forKey:@"userName"];
    [showDict setValue:myUserID forKey:@"userID"];
    //NSLog(@"showHand:%@",showHand);
    [showDict setValue:[showHand valueForKey:@"number"] forKey:@"handNumber"];
    [showDict setValue:[showHand valueForKey:@"communityCards"] forKey:@"communityCards"];
    for (NSMutableDictionary *round in [showHand valueForKey:@"rounds"]) {
        if([[round valueForKey:@"userID"] isEqualToString:myUserID]){
            [showDict setValue:[round valueForKey:@"cardOne"] forKey:@"cardOne"];
            [showDict setValue:[round valueForKey:@"cardTwo"] forKey:@"cardTwo"];
        }
    }
    ...
    NSMutableDictionary *localMessageDict = [[NSMutableDictionary alloc] init];
    [localMessageDict setValue:@"" forKey:@"message"];
    [localMessageDict setValue:@"-2" forKey:@"userID"];
    [localMessageDict setValue:@"Dealer" forKey:@"userName"];
    [localMessageDict setValue:currentGameID forKey:@"gameID"];
    [localMessageDict setValue:@"YES" forKey:@"isRead"];
    [localMessageDict setValue:[utcFormatter stringFromDate:[NSDate date]] forKey:@"created_at"];
    [localMessageDict setValue:showDict forKey:@"data"];
    [self updateGameMessages:localMessageDict];
    //[self incrementNewMessageCountBy:1];
    ...
    dispatch_async(kBgQueue, ^{
        ...
        [messageDict setValue:recipients forKey:@"recipients"];
        [messageDict setValue:@"" forKey:@"message"];
        [messageDict setValue:@"-2" forKey:@"userID"];
        [messageDict setValue:@"Dealer" forKey:@"userName"];
        [messageDict setValue:currentGameID forKey:@"gameID"];
        [messageDict setValue:showDict forKey:@"data"];
...
-(BOOL)canCurrentGameInvitePlayer{
    if([self isCurrentGameComplete]){
        return NO;
    }
    if ([self isCurrentGameActive] && [self isCurrentGameTypeTournament]) {
        return NO;
    }
    
    NSMutableDictionary *turnState = [currentGame objectForKey:@"turnState"];
    NSMutableArray *players = [turnState objectForKey:@"players"];
