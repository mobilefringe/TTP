...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        playNowLabel = [[UILabel alloc] initWithFrame:CGRectMake(85, 5, 190, 30)];
        playNowLabel.textColor = [UIColor colorWithRed:1 green:1 blue:.3 alpha:1];
        playNowLabel.textAlignment = UITextAlignmentLeft;
        playNowLabel.font = [UIFont  fontWithName:[Settings sharedInstance].header1Font size:22];
        ...
        playNowDescriptionLabel = [[UILabel alloc] initWithFrame:CGRectMake(85, 32, 180, 44)];
        playNowDescriptionLabel.textColor = [UIColor whiteColor];
        playNowDescriptionLabel.textAlignment = UITextAlignmentLeft;
        playNowDescriptionLabel.font = [UIFont  fontWithName:[Settings sharedInstance].header1Font size:11];
        ...
        self.feedbackImageView = [[UIImageView alloc] initWithFrame:CGRectMake(-20, 10, 90, 70)];
        feedbackImageView.image = [UIImage imageNamed:@"facebook_icon.png"];
...
-(void)setCellData:(NSMutableDictionary *)game{
    ...
    if(isPlayNow || isCreateGame || isFreeGames || isDeleteGameBlurb || isFeedback){
        ...
        if(isPlayNow){
            ...
            playNowLabel.text = @"5 Games Free!";
            playNowDescriptionLabel.text = @"Play in five active games at once for free! Additional games just 20 Pro Chips.";
            ...
            playNowLabel.text = @"Invite Friends!";
            playNowDescriptionLabel.text = @"Invite your Facebook friends to play TTP. See who the best poker player is!";
        }
        return;
    }
    //NSLog(@"game:%@",game);
    ...
    vsLabel.textColor = [UIColor colorWithRed:0.9 green:0.9 blue:0.9 alpha:1];
    if(playerCount == 0){
        //vsLabel.textColor = [UIColor colorWithRed:0.7 green:.1 blue:0.1 alpha:1];
    ...
    if([[DataManager sharedInstance] isGamePending:game]){
        ...
        NSString *playersTurnStr = [[DataManager sharedInstance] getPlayersTurnForGame:game];
        if([playersTurnStr isEqualToString:[DataManager sharedInstance].myUserName]){
            playersTurn.text = [NSString stringWithFormat:@"-- %@ Turn --",@"Your"];
        }else{
            playersTurn.text = [NSString stringWithFormat:@"-- %@'s Turn --",playersTurnStr];
        }
        if(playerCount == 0){
            playersTurn.text = @"-- Press or Swipe to Leave --";
        }
