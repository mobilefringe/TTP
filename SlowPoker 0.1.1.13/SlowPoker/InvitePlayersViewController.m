...
-(void)addPlayerToGame:(NSMutableDictionary *)addPlayer{
    ...
    NSMutableArray *players = [turnState objectForKey:@"players"];
    if([players count] == 6){
        UIAlertView *cantAddAlert = [[UIAlertView alloc] initWithTitle:@"6 Player Max" message:@"You have reached the maxium number of players. Please remove a player before adding a new one" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [cantAddAlert show];
        return;
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if(section == 0){
        ...
        CellHeaderGeneral *header = [[CellHeaderGeneral alloc] initWithFrame:CGRectMake(0, 0, 320, 35) title:@"This Game"];
        return header;
    }else if(section == 2){
        if([[DataManager sharedInstance].myFriends count] > 0){
            CellHeaderGeneral *header = [[CellHeaderGeneral alloc] initWithFrame:CGRectMake(0, 0, 320, 35) title:@"My Friends"];
            return header;
        }else{
            return nil;
        }
