...
-(void)scrollToBottom{
    ...
    if([[DataManager sharedInstance] getMessagesForCurrentGame] && [[[DataManager sharedInstance] getMessagesForCurrentGame] count] > 1){
        ...
        if ( [self.tableView numberOfSections] >= [[[DataManager sharedInstance] getMessagesForCurrentGame] count]-1  &&  [self.tableView numberOfRowsInSection:[[[DataManager sharedInstance] getMessagesForCurrentGame] count]-1] > 0 ){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (cell == nil) {
    ...
    if([[[DataManager sharedInstance] getMessagesForCurrentGame] count] > 0){
        ...
        }else if([[message valueForKey:@"userID"] isEqualToString:@"-1"]){
            cell.imgName = @"gray_bubble.png";
            cell.tipRightward = YES;
            cell.msgText = [NSString stringWithFormat:@"%@: %@",[message valueForKey:@"userName"], [message valueForKey:@"message"]];
        }else if([[message valueForKey:@"userID"] isEqualToString:@"-2"]){
            cell.imgName = @"gray_bubble.png";
            cell.tipRightward = YES;
            cell.msgText = [NSString stringWithFormat:@"%@: %@",[message valueForKey:@"userName"], [self getMessageForShowHand:[message valueForKey:@"data"]]];
            cell.showHand = [message valueForKey:@"data"];
        }else{
        ...
        if([[message valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID]){
                    cell.dateText = [NSString stringWithFormat:@"%@ said %@",@"You",   [timeIntervalFormatter stringForTimeIntervalFromDate:[NSDate date] toDate:createdAt]];
        }else{
                    cell.dateText = [NSString stringWithFormat:@"%@ said %@",[message valueForKey:@"userName"],   [timeIntervalFormatter stringForTimeIntervalFromDate:[NSDate date] toDate:createdAt]];
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSMutableDictionary *message = [[[DataManager sharedInstance] getMessagesForCurrentGame] objectAtIndex:indexPath.section];
    if([[message valueForKey:@"userID"] isEqualToString:@"-2"]){
        aMsg = [self getMessageForShowHand:[message valueForKey:@"data"]];
    }
...
-(NSString *)getMessageForShowHand:(NSMutableDictionary *)showDict{
    ...
    return [NSString stringWithFormat:@"'%@' is showing the table %@,%@ for hand #%@",[showDict valueForKey:@"userName"],[showDict valueForKey:@"cardOne"],[showDict valueForKey:@"cardTwo"],[showDict valueForKey:@"handNumber"]];
}
