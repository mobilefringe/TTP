...
-(void)setPlayerData:(NSMutableDictionary *)playerData isCurrentHand:(BOOL)isCurrentHand winners:(NSMutableArray *)winners chipStackState:(int)chipStackState dealerButtonCentre:(CGPoint)dealerButtonCentre{
   // NSLog(@"player:%@",playerData);
    ...
    if(![player valueForKey:@"status"] ||  [[NSString stringWithFormat:@"%@",[player valueForKey:@"status"]] length] == 0){
    ...
    BOOL isCurrentUsersTurn = NO;
    if(([[player valueForKey:@"status"] isEqualToString:@"pending"] || [[player valueForKey:@"status"] isEqualToString:@"buyin"] || [[player valueForKey:@"status"] isEqualToString:@"out"])){
        playerView.alpha = 0.6;
        cardOne.alpha = 0;
        cardTwo.alpha = 0;
        if( ([[player valueForKey:@"status"] isEqualToString:@"out"]  && ![[DataManager sharedInstance] isCurrentGameActive])){
            cardOne.alpha = 1;
            cardTwo.alpha = 1;
        }
        ...
        if([[[DataManager sharedInstance].currentGame valueForKey:@"nextActionForUserID"] isEqualToString:[player valueForKey:@"userID"]] && [[DataManager sharedInstance] isCurrentGameActive] && isCurrentHand){
            ...
            }else if([[player valueForKey:@"action"] isEqualToString:@"call"] ){
                actionLabel.text = [NSString stringWithFormat:@"Call $%@",[player valueForKey:@"callAmount"]];
            }else{
                if([[player valueForKey:@"status"] isEqualToString:@"playing"]){
                    actionLabel.text = NSLocalizedString([player objectForKey:@"action"], nil);
                }else{
    ...
    if([@"YES" isEqualToString:[player objectForKey:@"showCards"]] || showCards){
        ...
        cardOne.frame = CGRectMake(43, 82, 15, 25);
        cardTwo.frame = CGRectMake(59, 82, 15, 25);
    ...
    if([[player valueForKey:@"action"] isEqualToString:@"fold"]){
        avatar.alpha = 0.65;
        actionLabel.alpha = 0.7;
        if([[player valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID]){
            cardOne.hidden = NO;
            cardTwo.hidden = NO;
        }else{
    ...
    //NSLog(@"playerData:%@",playerData);
    //NSLog(@"currentHand state:%@",[[DataManager sharedInstance].currentHand valueForKey:@"state"]);
    if(![[player valueForKey:@"handState"] isEqualToString:[[DataManager sharedInstance].currentHand valueForKey:@"state"]] && !isCurrentUsersTurn){
        if(![[player valueForKey:@"status"] isEqualToString:@"buyin"] && ![[player valueForKey:@"status"] isEqualToString:@"pending"]){
            actionLabel.textColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.5];
        }
    }
