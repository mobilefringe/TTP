...
-(id)init{
	...
	if (self != nil) {
        ...
        if(![prefs arrayForKey:@"LOCAL_ACHIEVEMENTS"]){
            self.localAcievements = [[NSMutableArray alloc] init];
        }else{
            self.localAcievements = [NSMutableArray arrayWithArray:[prefs arrayForKey:@"LOCAL_ACHIEVEMENTS"]];
        }
        
        if(![prefs arrayForKey:@"LOCAL_TALLY"]){
            self.tally = [[NSMutableDictionary alloc] init];
        }else{
            self.tally = [NSMutableDictionary dictionaryWithDictionary:[prefs dictionaryForKey:@"LOCAL_TALLY"]];
        }
    }
    return self;
}
...
-(NSMutableDictionary *)updatePlayerPassword:(NSMutableDictionary *)playerData{
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSError *error = nil;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:playerData options:NSJSONWritingPrettyPrinted error:&error];
    ...
    NSMutableDictionary *results = [[APIDataManager sharedInstance] updatePlayerPassword:jsonString];
    //NSLog(@"results:%@",results);
    if([@"1" isEqualToString:[results objectForKey:@"success"]]){
        [prefs setObject:[playerData objectForKey:@"password"] forKey:@"password"];
        [self registerForAPN];
...
-(NSMutableDictionary *)updatePlayerEmail:(NSMutableDictionary *)playerData{
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSError *error = nil;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:playerData options:NSJSONWritingPrettyPrinted error:&error];
    ...
    //NSLog(@"results:%@",results);
    if([@"1" isEqualToString:[results objectForKey:@"success"]]){
        [prefs setObject:[playerData objectForKey:@"email"] forKey:@"email"];
        [self registerForAPN];
        [prefs synchronize];
    }
    return results;
...
-(void)postRound:(NSMutableDictionary *)round{
    ...
    for (int i = 0; i < rounds.count; i++) {
        ...
        if([[round valueForKey:@"action"] isEqualToString:@"raise"] && handState == 1 && ![[round valueForKey:@"userID"] isEqualToString:myUserID]){
            notMeRaise = YES;
        }
    }
...
-(void)createNewGame:(NSMutableDictionary *)game{
    //invite via APN
    [game setValue:@"isPrivate" forKey:@"true"];
    NSMutableArray *players = [[game objectForKey:@"turnState"] objectForKey:@"players"];
    NSMutableArray *invitePlayers = [[NSMutableArray alloc] init];
    for (NSMutableDictionary *player in players) {
        if(![[player valueForKey:@"userID"] isEqualToString:myUserID]){
...
-(void)incrementAchievement:(NSString *)code category:(NSString *)category earnedValue:(NSString *)earnedValue forUser:(NSString *)userID countInc:(int)countInc{
    ...
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    [prefs setObject:localAcievements forKey:@"LOCAL_ACHIEVEMENTS"];
...
-(int)incrementClientTally:(NSString *)code gameID:(NSString *)gameID{
    ...
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    [prefs setObject:tally forKey:@"LOCAL_TALLY"];
    [prefs synchronize];
    return tallyValue;
}
