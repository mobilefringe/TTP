...
-(void)loadView{
    ...
    listTableButton.frame = CGRectMake(3, 5, 40,30);
    [listTableButton addTarget:self action:@selector(flipTable) forControlEvents:UIControlEventTouchUpInside];
    [chatFieldBackground addSubview:listTableButton];
    
    self.chatField = [[UITextField alloc] initWithFrame:CGRectMake(50, 5, 320-115,30)];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if([[[NSUserDefaults standardUserDefaults] valueForKey:@"isTableView"] isEqualToString:@"YES"]){
        ...
        [listTableButton setTitle:@"V2" forState:UIControlStateNormal];
    }else {
        [pokerTableView removeFromSuperview];
        [flipView  addSubview:_tableView];
        navLabel1.text = @"Bet View";
        [listTableButton setTitle:@"V1" forState:UIControlStateNormal];
    }
...
-(void)showGameOverIfNeeded{
    ...
    if([[DataManager sharedInstance] isCurrentGameComplete] || [[[[DataManager sharedInstance] getPlayerMeForCurrentGame] valueForKey:@"status"] isEqualToString:@"out"]){
        if(!gameOverAlert){
            self.gameOverAlert = [[UIAlertView alloc] initWithTitle:@"Game Over" message:@"" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        }
        ...
        for (NSMutableDictionary *playerSummary in gameWinners) {
...
-(void)hideChat{
    ...
    messageBubblesViewController.view.frame = CGRectMake(0, 416-40, 320, 416-40);
    chatField.frame = CGRectMake(50, 5, 320-115,30);
    [UIView commitAnimations];
