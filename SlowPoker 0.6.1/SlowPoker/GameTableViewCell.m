...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        playNowLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 10, 200, 30)];
        playNowLabel.textColor = [UIColor darkGrayColor];
        playNowLabel.textAlignment = UITextAlignmentLeft;
        playNowLabel.font = [UIFont boldSystemFontOfSize:22];
        ...
        playNowDescriptionLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 35, 250, 40)];
        playNowDescriptionLabel.textColor = [UIColor lightGrayColor];
        playNowDescriptionLabel.textAlignment = UITextAlignmentLeft;
        playNowDescriptionLabel.font = [UIFont systemFontOfSize:13];
        playNowDescriptionLabel.adjustsFontSizeToFitWidth = YES;
        playNowDescriptionLabel.numberOfLines = 2;
...
-(void)setCellData:(NSMutableDictionary *)game{
    ...
    if(isPlayNow){
        ...
        playNowDescriptionLabel.text = @"Play in a random game and meet new friends!";
        return;
    }
    //NSLog(@"game:%@",game);
        ...
        if([[playerMe objectForKey:@"playerState"] valueForKey:@"userStack"]){
            ...
            if([[[playerMe objectForKey:@"playerState"] valueForKey:@"userStack"] doubleValue] == 0){
                ...
                if([@"tournament" isEqualToString:gameType] && [[DataManager sharedInstance] isGameComplete:game]){
                    userStackLabel.textColor = [UIColor colorWithRed:0.1 green:.7 blue:0.1 alpha:1];
