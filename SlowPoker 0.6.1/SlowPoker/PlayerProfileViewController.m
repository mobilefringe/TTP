...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
       self.title = @"Profile";
       [[DataManager sharedInstance] addObserver:self forKeyPath:@"playerProfile" options:NSKeyValueObservingOptionOld context:nil];
    }
    return self;
}
...
{
    if([keyPath isEqualToString:@"playerProfile"]){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section == 0){
        ...
        [cell setProfileData:[DataManager sharedInstance].playerProfile];
        if([[[DataManager sharedInstance].playerProfile valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID]){
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if(section == 0){
        if(![[[DataManager sharedInstance].playerProfile valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID]){
            ...
            footerView1.text = @"Upload avatar";
        }
        return footerView1;
    }else if(section == 1){
        footerView2.text = @"View all stats";
        return footerView2;
    }else if(section == 2){
        footerView3.text = @"View achievements";
        return footerView3;
    }
    return nil;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section{
    if(![[[DataManager sharedInstance].playerProfile valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID] && section == 0){
        return 0;
    }
    return 18;
}
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if([[[DataManager sharedInstance].playerProfile valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID] && indexPath.section == 0){
