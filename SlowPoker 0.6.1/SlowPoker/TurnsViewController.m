...
-(void)loadView{
    ...
    background.frame = CGRectMake(0, -40, 320, 456);
    ...
    self.flipView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 416-40)];
    ...
    [flipView addSubview:_tableView];
    
    self.pokerTableView = [[PokerTableView alloc] initWithFrame:CGRectMake(0, 0, 320, 416-40)];
    ...
    self.listTableButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    listTableButton.frame = CGRectMake(3, 5, 100,30);
    [listTableButton setTitle:@"Game Stats" forState:UIControlStateNormal];
    [listTableButton addTarget:self action:@selector(showGameStats) forControlEvents:UIControlEventTouchUpInside];
    [chatFieldBackground addSubview:listTableButton];
    
    self.chatField = [[UITextField alloc] initWithFrame:CGRectMake(110, 5, 320-115-60,30)];
    ...
    self.handSummaryPopUp = [[HandSummaryPopUp alloc] initWithFrame:CGRectMake(5, 20, 310, 350) delegate:self];
...
-(void)resetGameStats{
    gameStatsViewController.view.frame = CGRectMake(0, 460, 320, 416);
}
...
-(void)showWinnerLoserIfNeeded{
    if(([winnerLoserAlert isVisible] || [winnerLoserAlertWithBuyin isVisible]) && !showhandOver){
        return;
    }
            ...
            if(winners && [winners count] > 0){
                for (NSMutableDictionary *winnerPlayerData in winners) {
                    ...
                    if([[winnerPlayerData valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID]){
                        ...
                        int handsWon = [[DataManager sharedInstance] incrementClientTally:@"HANDS_WON" gameID:@"-1"];
...
-(void)showGameOverIfNeeded{
    if([gameOverAlert isVisible] || !showGameOver){
        return;
    }
    if([[DataManager sharedInstance] isCurrentGameComplete] || [[[[DataManager sharedInstance] getPlayerMeForCurrentGame] valueForKey:@"status"] isEqualToString:@"out"]){
        if(!gameOverAlert){
            self.gameOverAlert = [[UIAlertView alloc] initWithTitle:@"Game Over" message:@"" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Game Summary", nil];
        }
        ...
        if([[winner valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID]){
            ...
            if([prefs valueForKey:gameKey] && [@"YES" isEqualToString:[prefs valueForKey:gameKey]]){
                ...
                int gamesWon = [[DataManager sharedInstance] incrementClientTally:@"GAMES_WON" gameID:@"-1"];
                NSMutableDictionary *gameAchievement = [[DataManager sharedInstance] getGameAchievementForCode:@"GAME_WIN_TOTAL" category:@"BRONZE"];
                [[DataManager sharedInstance] incrementAchievement:gameAchievement forUser:[DataManager sharedInstance].myUserID];
                if(gamesWon == 1){
                ...
                if(gamesWon%5==0){
                    NSMutableDictionary *gameAchievement = [[DataManager sharedInstance] getGameAchievementForCode:@"GAME_WIN_TOTAL_5" category:@"SILVER"];
                    [[DataManager sharedInstance] incrementAchievement:gameAchievement forUser:[DataManager sharedInstance].myUserID];
                    
                }
                if(gamesWon%50==0){
                    NSMutableDictionary *gameAchievement = [[DataManager sharedInstance] getGameAchievementForCode:@"GAME_WIN_TOTAL_25" category:@"GOLD"];
                    [[DataManager sharedInstance] incrementAchievement:gameAchievement forUser:[DataManager sharedInstance].myUserID];
...
-(void)hideChat{
    ...
    messageBubblesViewController.view.frame = CGRectMake(0, 416-40, 320, 416-40);
    chatField.frame = CGRectMake(110, 5, 320-115-60,30);
    [UIView commitAnimations];
...
-(void)showChatFull{
    ...
    [UIView beginAnimations:@"" context:NULL];
    [UIView setAnimationDuration:0.2];
    chatField.frame = CGRectMake(5, 5, 320-70,30);
...
-(void)flipTable{
    ...
    if ([_tableView superview]) {
        ...
        [UIView setAnimationTransition:UIViewAnimationTransitionFlipFromRight forView:flipView
                                 cache:YES]; 
        [pokerTableView removeFromSuperview];
        [flipView  insertSubview:_tableView belowSubview:activityIndicatorView];
        navLabel1.text = @"Bet View";
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:0];
        [_tableView scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionTop animated:NO];
        [UIView setAnimationDelegate:self];
