...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section == 0){
        ...
        if(indexPath.row == 0){
            ...
            if(!resetProChips){
                self.resetProChips = [[UIAlertView alloc] initWithTitle:@"Reset Pro Chips" message:[NSString stringWithFormat:@"Reset Pro Chips to zero?",[DataManager sharedInstance].myUserName] delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Reset", nil];
            }
            [resetProChips show];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if(section == 0){
        CellHeaderGeneral *header = [[CellHeaderGeneral alloc] initWithFrame:CGRectMake(0, 0, 320, 30) title:@"About TTP"];
        return header;
    }else if(section == 1){
        CellHeaderGeneral *header = [[CellHeaderGeneral alloc] initWithFrame:CGRectMake(0, 0, 320, 30) title:@"Settings"];
        return header;
    }else if(section == 2){
        CellHeaderGeneral *header = [[CellHeaderGeneral alloc] initWithFrame:CGRectMake(0, 0, 320, 30) title:@"Test"];
        return header;
    }
    return nil;
