...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        background.frame = CGRectMake(isIphoneXOrPlus?0:-10, -10,isIphoneXOrPlus ?  [UIScreen mainScreen].bounds.size.width-22 :  [UIScreen mainScreen].bounds.size.width, 110);
        [self.contentView addSubview:background];
        
        self.avatar = [[Avatar alloc] initWithFrame:CGRectMake(isIphoneXOrPlus?22:12, 9, 80, 71)];
        avatar.radius = 130;
        ...
        self.userIDLabel = [[UILabel alloc] initWithFrame:CGRectMake(210, 3, 90, 15)];
        ...
        self.locationLabel = [[UILabel alloc] initWithFrame:CGRectMake(110, 28, 185, 18)];
        ...
        self.playerSinceLabel = [[UILabel alloc] initWithFrame:CGRectMake(110, 43, 185, 20)];
...
-(void)setProfileData:(NSMutableDictionary *)playerProfieDict isFriend:(BOOL)isFriend{
    ...
    if([[playerProfieDict valueForKey:@"userID"] isEqualToString:[DataManager sharedInstance].myUserID]){
        addFriendsLabel.text = @"Update Avatar";
    }else{
        if(isFriend){
            addFriendsLabel.text = @"- Remove as friend";
        }else{
            addFriendsLabel.text = @"+ Add as friend";
        }
    }
}
