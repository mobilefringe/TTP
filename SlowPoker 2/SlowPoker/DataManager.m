...
-(void)fetchedNewMessages:(NSMutableDictionary *)newMessages{
    ...
    NSString *gameMessageCountKey = [NSString stringWithFormat:@"newMessagesCount_%@_%@",currentGameID,myUserID];
    ...
    if([[newMessages valueForKey:@"messages"] count] > 0){
        AudioServicesPlaySystemSound (kSystemSoundID_Vibrate);
        //NSLog(@"new Messages:%@",newMessages);
    }
}

-(void)updateNewMessageCount:(int)newMessagesTmp{
    NSString *gameMessageCountKey = [NSString stringWithFormat:@"newMessagesCount_%@_%@",currentGameID,myUserID];
    self.newMessagesCount = newMessagesTmp;
    [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%d", newMessagesCount] forKey:gameMessageCountKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
        //NSLog(@"[[NSUserDefaults standardUserDefaults] objectForKey:gameMessageCountKey]:%@",[[NSUserDefaults standardUserDefaults] objectForKey:gameMessageCountKey]);
}

-(void)incrementNewMessageCountBy:(int)messageIncrement{
    NSString *gameMessageCountKey = [NSString stringWithFormat:@"newMessagesCount_%@_%@",currentGameID,myUserID];
    [self updateNewMessageCount: (messageIncrement + [[[NSUserDefaults standardUserDefaults] valueForKey:gameMessageCountKey] doubleValue]) ];
