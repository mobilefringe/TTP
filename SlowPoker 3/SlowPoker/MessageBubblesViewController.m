...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
  return [[DataManager sharedInstance].gameMessages count];
}
...
-(void)scrollToBottom{
    [self.tableView reloadData];
    if([DataManager sharedInstance].gameMessages && [[DataManager sharedInstance].gameMessages count] > 1){
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:[[DataManager sharedInstance].gameMessages count]-1 ];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (cell == nil) {
    ...
    NSMutableDictionary *message = [[DataManager sharedInstance].gameMessages objectAtIndex:indexPath.section];
    //NSLog(@"message:%@",message);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSString *aMsg = [[[DataManager sharedInstance].gameMessages valueForKey:@"message"] objectAtIndex:indexPath.section];
