...
-(void)loadView{
    ...
    chatButton.frame = CGRectMake(258, 5, 60, 30);
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.refreshTimer = [NSTimer scheduledTimerWithTimeInterval:5.0 target:self selector:@selector(loadGame) userInfo:nil repeats:YES];
    [refreshTimer fire];
    ...
    if(isGameComplete){
        ...
        if([DataManager sharedInstance].newMessagesCount > 0){
            if(messageBubblesViewController.view.frame.origin.y == 416-40){
                ...
                //[chatButton setTitle:[NSString stringWithFormat:@"Chat(%d)",[DataManager sharedInstance].newMessagesCount] forState:UIControlStateNormal];
            }
        }   
    }
}
...
-(void)showBuyInAlertIfNeeded{
    ...
    NSMutableDictionary *currentPlayer = [[DataManager sharedInstance] getCurrentPlayerForCurrentGame];
    if([@"pending" isEqualToString:[currentPlayer objectForKey:@"status"]] && [[DataManager sharedInstance] isCurrentGameTypeCash]){
        double minBuy = [[[[DataManager sharedInstance].currentGame objectForKey:@"gameSettings"] objectForKey:@"minBuy"] doubleValue];
        double maxBuy = [[[[DataManager sharedInstance].currentGame objectForKey:@"gameSettings"] objectForKey:@"maxBuy"] doubleValue];
        buyInAlert.title = @"Buy In?";
        buyInRange.text = [NSString stringWithFormat:@"Min $%.2f / Max $%.2f",minBuy,maxBuy];
    ...
    if([@"pending" isEqualToString:[[DataManager sharedInstance].currentGame objectForKey:@"status"]]){
        ...
        if(![@"pending" isEqualToString:[currentPlayer objectForKey:@"status"]] && ![[DataManager sharedInstance] isGameWaitingForOtherPlayers:[DataManager sharedInstance].currentGame]){
            ...
            if([[DataManager sharedInstance] isCurrentGameOwnerMe]){
                ...
                UIAlertView *waitingForGameOwner = [[UIAlertView alloc] initWithTitle:@"Waiting for Game Owner" message:@"Please wait for the game owner to start the game" delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            ...
            if(![@"pending" isEqualToString:[currentPlayer objectForKey:@"status"]]){
                if([[DataManager sharedInstance] isCurrentGameTypeCash]){
                    ...
                    self.notEnoughPlayersAlert = [[UIAlertView alloc] initWithTitle:@"Waitig for Players" message:[NSString stringWithFormat:@"%d/%d players are ready. Please wait for all players to join the tournament or edit the player list to proceed",numOfReadyPlayers,players.count] delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK",@"Edit players", nil];
                }
    ...
    if([hands count] > 0){
        ...
        if([prefs valueForKey:handKey] && [@"YES" isEqualToString:[prefs valueForKey:handKey]]){
            ...
            if(winners && [winners count] > 0){
                ...
                if([[myTurnState valueForKey:@"userStack"] doubleValue] == 0){
...
-(void)pressBet{
    [betOptions showInView:self.view];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    /*
    if([round objectForKey:@"MY_BET_ROUND"] && [@"YES" isEqualToString:[round objectForKey:@"MY_BET_ROUND"]]){
        betButton.hidden = NO;
...
-(void)hideChat{
    chatField.text = @"";
    chatField.placeholder = @"Game Chat";
    [DataManager sharedInstance].newMessagesCount = 0;
    self.navigationItem.rightBarButtonItem = bugButton;
    [UIView beginAnimations:@"" context:NULL];
