...
-(void)loadView{
    ...
    UIImageView *background = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg_cage.png"]];
    background.frame = CGRectMake(0, -50, 320, 480);
...
-(void)viewWillAppear:(BOOL)animated{
    [DataManager sharedInstance].isGamesUpdating = NO;
...
-(void)viewWillDisappear:(BOOL)animated{
    [[DataManager sharedInstance] removeObserver:self forKeyPath:@"gamesUpdated" context:nil];
    [[DataManager sharedInstance] removeObserver:self forKeyPath:@"joinGameID" context:nil];
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section == 0){
        if(indexPath.row == [[DataManager sharedInstance].yourTurnGames count]){
            ...
            if(!joinGameAlert){
                self.joinGameAlert = [[UIAlertView alloc] initWithTitle:@"Random Table" message:@"Would you like to join a game with random players?" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"OK", nil];
            }
            [joinGameAlert show];
...
-(void)loadGame:(NSString *)gameID{
    if(gameID){
        ...
        [turnsViewController resetGameStats];
        turnsViewController.fromBetView = NO;
